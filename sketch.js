...
textSize(12*s_x)
text("20",560*s_x,252*s_y);
text("-20",560*s_x,351*s_y);
...
fill(102, 194, 255,180);
textSize(12*s_x)
text("5",798*s_x,313*s_y);
text("-5",298*s_x,313*s_y);
...
fill(102, 194, 255,180);
textSize(14)
text("5",798*s_x,315*s_y);
text("5",298*s_x,315*s_y);
...
///////
  ...
  for (let k = -1000; k < 0; k++) {
    ...
    let n = Math.pow(Math.pow(10,20)/hole_add_new,exp(V_applied_p/0.026-1)*exp((k+550+((400)/8*count_pn_num))*s_x))
    // console.log (n+"hellpppppppp")
    // if (x<(550-((400)/8*X_n)*s_x)){

      new_array_plot_e_set_count.push({ x: x, y: (n-1)*1 });
    // }
...
//   let startY = 171.25 * s_y; // starting Y coordinate
// let startX = 190 * s_x; // starting X coordinate

// let x = 190 * s_x+ i ; // X coordinate increments by 1 * s_x each time
//   let y = startY + new_array_plot_e_set_count[i]; // Y coordinate is the starting Y plus the cumulative total


  ellipse(190*s_x+i, 171.25 * s_y-new_array_plot_e_set_count[i].y*Math.pow(10,12),3);
}
